	...
	**/
	if (isset($_POST['book_id'])          &&
	    ...
	    **/
        ...
        $location = "../edit-book.php";
        $ms = "id=$id&error";
		is_empty($recommended, $text, $location, $ms, "");
        /**
          if the admin try to 
		        ...
		        if ($book_cover['status'] == "error" || 
		          ...
		          **/
		           ...
		           $book_cover_URL = $book_cover['data'];
					...
					$res  = $stmt->execute([$title, $author, $description, $category,$recommended,$book_cover_URL, $file_URL, $id]);
		           ...
		           $book_cover_URL = $book_cover['data'];
		            ...
		            # update just the data
		          	$sql = "UPDATE books
		          	        ...
		          	        WHERE id=?";
		          	$stmt = $conn->prepare($sql);
					$res  = $stmt->execute([$title, $author, $description, $category,$recommended,$book_cover_URL, $id]);
	        ...
	        if ($file['status'] == "error") {
	          ...
	          **/
	           $file_URL = $file['data'];
	            ...
	            # update just the data
	          	$sql = "UPDATE books
	          	        ...
	          	        WHERE id=?";
	          	$stmt = $conn->prepare($sql);
				$res  = $stmt->execute([$title, $author, $description, $category ,$recommended, $file_URL, $id,]);
          	...
          	$sql = "UPDATE books
          	        ...
          	        WHERE id=?";
          	$stmt = $conn->prepare($sql);
			$res  = $stmt->execute([$title, $author, $description, $category,$recommended, $id,]);
